...
#creating a csv
filename = 'Exhibitor_List_with_Jobs.csv'
f = open(filename, "w", encoding="utf-8")
#creating titles of each column, note the \n
headers= "Company Name,Website,Location,Careers?\n"
f.write(headers)
...
for exhibitor in range(2, 205):
    ...
    #going into each individual companies website to see if they are hiring
    jobs = 'Skipped/No'
    try:
        uClient = uReq('https://' + website)
        page_html = uClient.read()
        uClient.close()
        individual_website = str(soup(page_html, "html.parser")) #string because we are just searching for the keywords
        if individual_website.find("careers" or "jobs"):
            jobs = "Yes"
        else:
            jobs = "No"    
    except:
        pass #used in case a website doesn't allow bs4 querying      
    f.write(company_name.replace(',', '') + "," + website.replace(',', '') + "," + location.replace(',', '') + "," + jobs + "\n")
